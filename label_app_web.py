...
def _load_image(img_ref):
    ...
    if os.path.exists(str(img_ref)): return img_ref
    local = os.path.join(IMAGE_DIR, os.path.basename(str(img_ref)))
...
def generate_pdf_bytes(selected_items, field_config, global_sup_bytes, page_format="A4"):
    ...
    try:
        ...
        # Calcular cuántos rótulos caben por página con gap mínimo de 2mm
        GAP_MIN      = 2 * mm
        max_per_page = max(1, int((USABLE_H + GAP_MIN) / (LABEL_H + GAP_MIN)))
        # Agrupar en páginas
        pages = [selected_items[i:i+max_per_page] for i in range(0, len(selected_items), max_per_page)]

        for page_items in pages:
            n = len(page_items)
            # Distribuir espacio sobrante entre los gaps (solo entre rótulos)
            # Rótulos empiezan desde el margen superior
            total_lbl_h  = n * LABEL_H
            sobrante     = USABLE_H - total_lbl_h  # espacio libre total
            n_gaps       = max(1, n - 1)            # gaps entre rótulos
            gap_pts      = int(sobrante / n_gaps)   # gap óptimo en puntos
            # El resto (<1 gap) queda al final — mínimo 2mm entre rótulos
            gap_pts      = max(2 * mm, gap_pts)

            for row, item in enumerate(page_items):
                # Empieza desde el margen superior, gaps solo entre rótulos
                y_from_top = MARGIN + row * (LABEL_H + gap_pts)
...
with tab_preview:
    ...
    if st.button("📄 Generar y Descargar PDF", type="primary", disabled=not selected_items_all):
        ...
        if err:
            ...
            fname = f"rotulos_{fmt_code}_{datetime.date.today().strftime('%Y%m%d')}.pdf"
