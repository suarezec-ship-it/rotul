...
def generate_pdf_bytes(selected_items, field_config, global_sup_bytes, page_format="A4"):
    ...
    try:
        ...
        if page_format == "35x20cm":
            # 35cm ancho x 20cm alto (paisaje)
            PAGE_W = 35 * mm * 10   # 350mm en puntos
            PAGE_H = 20 * mm * 10   # 200mm en puntos
            LABEL_H = 45 * mm       # ajustar alto de rótulo para que quepan
            # Con 200mm de alto y 3mm margen → usable 194mm
            # rótulo de 45mm → caben 4 por página (4×45=180mm)
        else:
            # A4 estándar
        ...
        # Calcular cuántos rótulos caben por página
        max_per_page = max(1, int(USABLE_H / LABEL_H))
        # Agrupar en páginas
...
with tab_preview:
    ...
    with fmt_col1:
        page_format = st.selectbox(
            "📐 Formato de papel",
            ["A4 (210×297mm)", "35×20cm (personalizado)"],
            key="page_format_sel"
        )
        fmt_code = "35x20cm" if "35×20" in page_format else "A4"

    if selected_items_all:
        n_rotulos    = len(selected_items_all)
        per_pag      = 4 if fmt_code == "35x20cm" else 6
