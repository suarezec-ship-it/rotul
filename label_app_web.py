...
def generate_pdf_bytes(selected_items, field_config, global_sup_bytes, page_format="A4"):
    ...
    try:
        ...
        GAP_FIXED = 3 * mm  # gap fijo entre rótulos — siempre igual

        for page_items in pages:
            for row, item in enumerate(page_items):
                # Siempre desde el margen superior, gap fijo de 3mm
...
with tab_list:
    ...
    def _find_duplicates(item_list):
        import re as _re2
        seen, dups = {}, []
    ...
    if st.session_state.get('show_dups', False):
        dup_indices = _find_duplicates(items)
        if not dup_indices:
            st.success("✅ No hay duplicados.")
        else:
            st.warning(f"⚠️ **{len(dup_indices)}** posibles duplicados encontrados:")
            dup_to_delete = []
            for di in dup_indices:
                it = items[di]
                dc1, dc2, dc3 = st.columns([0.5, 8, 1.5])
                with dc1:
                    if st.checkbox("", key=f"dup_{di}", value=True):
                        dup_to_delete.append(di)
                with dc2:
                    st.markdown(
                        f"**#{di}** {it.get('product','')[:80]}  \n"
                        f"Lote: `{it.get('batch','')}` · F.EXP: {it.get('exp_date','')}",
                        unsafe_allow_html=True)
                with dc3:
                    if st.button("✏️", key=f"dup_ed_{di}", help="Editar"):
                        st.session_state.selected_idx = di; st.rerun()
            if dup_to_delete:
                if st.button(f"🗑 Eliminar {len(dup_to_delete)} seleccionados", type="primary"):
                    cur = _get_items()
                    cur = [it for i, it in enumerate(cur) if i not in set(dup_to_delete)]
                    _set_items(cur); save_items()
                    st.session_state['show_dups'] = False
                    st.success(f"✅ {len(dup_to_delete)} duplicados eliminados"); st.rerun()
