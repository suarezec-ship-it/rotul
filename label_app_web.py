...
def generate_pdf_bytes(selected_items, field_config, global_sup_bytes, page_format="A4"):
    ...
    try:
        ...
        for page_items in pages:
            n = len(page_items)
            # Gap simétrico: espacio sobrante dividido en (n+1) gaps iguales
            total_labels_h = n * LABEL_H
            gap_space      = USABLE_H - total_labels_h
            sym_gap        = gap_space / (n + 1) if n > 0 else 0

            for row, item in enumerate(page_items):
                # Posición desde el borde superior: margen + gap_inicial + fila*(alto+gap)
                y_from_top = MARGIN + sym_gap + row * (LABEL_H + sym_gap)
